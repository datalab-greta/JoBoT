...
# -*- coding: utf-8 -*-
"""
Created on Mon Sep 16 17:30:53 2019

@author: Maxime & cyprien
"""
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
...
Mots_cles VARCHAR(150) NULL DEFAULT NULL,
...
#Insertion du dico dans la BDD
        ...
        param={'ref':ref,'title':title[:200],'employeur':entreprise[:250],'localite':address[:250],'contrat':contrat[:200],'temps_job':temps_job[:35],'salaire':salaire[:80],'savoirs':str(skills).strip('[]')[:1000],'savoir_etre':str(savoir_etre_pro).strip('[]').replace("'","")[:1000],'qualif':statut[:50],'exp':xp[:50],'formation':formation[:100],'secteur':secteur[:200],'lien_tiers':partenaire[:200],'provenance':origin[:50],'contact':contact[:250],'description':description[:5000],'motscles':str(motclef)[:100]}
        s=text("""
INSERT INTO AnnoncesPE(Reference, Intitule, Employeur, Localite, Date_MIEL, Date_MAJ,Contrat,Temps_travail,Salaire,Savoirs,Savoir_etre,Qualification,Expérience,Formation,Secteur_activite,Lien_partenaire,Provenance,Contact,Description,Mots_cles)
VALUES(:ref,:title,:employeur,:localite,CURRENT_DATE(),CURRENT_DATE(),:contrat,:temps_job,:salaire,:savoirs,:savoir_etre,:qualif,:exp,:formation,:secteur,:lien_tiers,:provenance,:contact,:description,:motscles)
ON DUPLICATE KEY
UPDATE
Intitule = :title,
...
autocsv = "/home/lebrun/jobbot/Backup_JobBot/BackUpAnnoncesPE"+maintenant+".csv"
